...
import com.zeoflow.password.strength.resources.BestMatches;
...
import com.zeoflow.password.strength.scoring.Result;
import com.zeoflow.password.strength.scoring.TimeEstimate;
...
{
    private static final StartIndexComparator comparator = new StartIndexComparator();
    ...
    {
        this.configuration = new ConfigurationBuilder().createConfiguration(context);
    }
     ...
     */
    public PasswordChecker(Configuration configuration)
    {
    ...
    public static Double getEntropyFromGuesses(final BigDecimal guesses)
    {
        ...
        guesses_tmp = Double.isInfinite(guesses_tmp) ? Double.MAX_VALUE : guesses_tmp;
    ...
    private static void printEstimationInfo(final PasswordChecker passwordChecker, final String password)
    {
        ResourceBundle resourceBundle = ResourceBundle.getBundle("main", passwordChecker.getConfiguration().getLocale());
    ...
    private List<Match> getBestCombination(final Configuration configuration, final String password)
    {
        ...
        if (all_matches.size() == 0 || isRandom(password, good_enough_matches))
        {
    ...
    private void generateMatches(final long start_time, final String password, final Match match, final Map<Match, List<Match>> non_intersecting_matches, final Map<Integer, Match> brute_force_matches, final List<Match> matches, int matches_length, BestMatches best_matches) throws TimeoutException
    {
        ...
        for (Match next_match : Objects.requireNonNull(non_intersecting_matches.get(match)))
        {
            if (matches.size() > 1)
            {
    ...
    private boolean isRandom(final String password, final List<Match> matches)
    {
        ...
        if (matched_length < (password.length() * 0.5))
        {
            ...
            return matched_length < (password.length() * 0.8) && password.length() * 0.25 > max_matched_length;
    }
